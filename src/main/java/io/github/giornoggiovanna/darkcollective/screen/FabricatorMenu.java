...
public class FabricatorMenu extends AbstractContainerMenu {
    ...
    public FabricatorMK1Entity blockEntity;

    private Level level;
    private ContainerData data;

    public FabricatorMenu(int containerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(MenuTypes.FAB_MENU.get(), containerId);
        checkContainerSize(inv, 7);
        blockEntity = ((FabricatorMK1Entity) entity);
        this.level = inv.player.level();
        this.data = data;

        if(entity instanceof FabricatorMK1Entity be){
            this.blockEntity = be;
        }else{
            throw new IllegalStateException("Incorrect block entity class (%s) passed into FabricatorMenu!"
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                player, BlockInit.FABRICATORMK1.get());
    }
